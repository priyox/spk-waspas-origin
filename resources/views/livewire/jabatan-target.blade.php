                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            @endforelse
                                ...
                                <x-modal name="jabatan-modal" :show="$isModalOpen" maxWidth="lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <form wire:submit.prevent="store">
                            <div class="space-y-4">
                                ...
                                <!-- Eselon -->
                                <div>
                                    <x-input-label for="id_eselon" :value="__('Eselon')" />
                                    <select id="id_eselon" wire:model="id_eselon" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                        @foreach($eselons as $eselon)
                                            <option value="{{ $eselon->id }}">{{ $eselon->eselon }}</option>
                                        @endforeach
                                    </select>
                                    <x-input-error :messages="$errors->get('id_eselon')" class="mt-2" />
                                </div>

                                <!-- Bidang Ilmu -->
                                <div>
                                    <x-input-label for="id_bidang" :value="__('Bidang Ilmu')" />
                                    <select id="id_bidang" wire:model="id_bidang" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                        @foreach($bidangIlmus as $bidang)
                                            <option value="{{ $bidang->id }}">{{ $bidang->bidang }}</option>
                                        @endforeach
                                    </select>
                                    <x-input-error :messages="$errors->get('id_bidang')" class="mt-2" />
                                </div>
                            </div>

    ...
    <nav class="nav flex-column p-2">
        @foreach($menu as $item)
            ...
            <a class="nav-link d-flex justify-content-between align-items-center {{ $isActive ? 'active font-weight-bold' : 'text-dark' }}" href="{{ $href }}">
                <span><i class="{{ $item['icon'] ?? 'fa fa-circle' }} mr-2"></i> {{ $item['title'] }}</span>
                @if($hasChildren)
            ...
            @if($hasChildren)
                <div class="pl-3">
                    @foreach($item['children'] as $child)
                        ...
                        @endphp
                        <a class="nav-link small text-muted" href="{{ $childHref }}"> <i class="{{ $child['icon'] ?? 'fa fa-circle' }} mr-2"></i> {{ $child['title'] }}</a>
                    @endforeach
                </div>
            @endif
        @endforeach
